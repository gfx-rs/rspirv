...
impl Builder {
    ...
    pub fn ext_inst(&mut self, result_type: spirv::Word, set: spirv::Word, instruction: u32, operands: Vec<spirv::Word>) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn function_call(&mut self, result_type: spirv::Word, function: spirv::Word, arguments: Vec<spirv::Word>) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn image_texel_pointer(&mut self, result_type: spirv::Word, image: spirv::Word, coordinate: spirv::Word, sample: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn load(&mut self, result_type: spirv::Word, pointer: spirv::Word, memory_access: Option<spirv::MemoryAccess>) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn store(&mut self, pointer: spirv::Word, object: spirv::Word, memory_access: Option<spirv::MemoryAccess>) -> BuildResult<()> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
    ...
    pub fn copy_memory(&mut self, target: spirv::Word, source: spirv::Word, memory_access: Option<spirv::MemoryAccess>) -> BuildResult<()> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
    ...
    pub fn copy_memory_sized(&mut self, target: spirv::Word, source: spirv::Word, size: spirv::Word, memory_access: Option<spirv::MemoryAccess>) -> BuildResult<()> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
    ...
    pub fn access_chain(&mut self, result_type: spirv::Word, base: spirv::Word, indexes: Vec<spirv::Word>) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn in_bounds_access_chain(&mut self, result_type: spirv::Word, base: spirv::Word, indexes: Vec<spirv::Word>) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn ptr_access_chain(&mut self, result_type: spirv::Word, base: spirv::Word, element: spirv::Word, indexes: Vec<spirv::Word>) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn array_length(&mut self, result_type: spirv::Word, structure: spirv::Word, array_member: u32) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn generic_ptr_mem_semantics(&mut self, result_type: spirv::Word, pointer: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn in_bounds_ptr_access_chain(&mut self, result_type: spirv::Word, base: spirv::Word, element: spirv::Word, indexes: Vec<spirv::Word>) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn vector_extract_dynamic(&mut self, result_type: spirv::Word, vector: spirv::Word, index: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn vector_insert_dynamic(&mut self, result_type: spirv::Word, vector: spirv::Word, component: spirv::Word, index: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn vector_shuffle(&mut self, result_type: spirv::Word, vector_1: spirv::Word, vector_2: spirv::Word, components: Vec<u32>) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn composite_construct(&mut self, result_type: spirv::Word, constituents: Vec<spirv::Word>) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn composite_extract(&mut self, result_type: spirv::Word, composite: spirv::Word, indexes: Vec<u32>) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn composite_insert(&mut self, result_type: spirv::Word, object: spirv::Word, composite: spirv::Word, indexes: Vec<u32>) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn copy_object(&mut self, result_type: spirv::Word, operand: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn transpose(&mut self, result_type: spirv::Word, matrix: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn sampled_image(&mut self, result_type: spirv::Word, image: spirv::Word, sampler: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn image_sample_implicit_lod(&mut self, result_type: spirv::Word, sampled_image: spirv::Word, coordinate: spirv::Word, image_operands: Option<spirv::ImageOperands>) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn image_sample_explicit_lod(&mut self, result_type: spirv::Word, sampled_image: spirv::Word, coordinate: spirv::Word, image_operands: spirv::ImageOperands) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
        let inst = mr::Instruction::new(spirv::Op::ImageSampleExplicitLod, Some(result_type), Some(id), vec![mr::Operand::IdRef(sampled_image), mr::Operand::IdRef(coordinate), mr::Operand::ImageOperands(image_operands)]);
        self.basic_block.as_mut().unwrap().instructions.push(inst);
        Ok(id)
    }

    /// Appends an OpImageSampleDrefImplicitLod instruction to the current basic block.
    pub fn image_sample_dref_implicit_lod(&mut self, result_type: spirv::Word, sampled_image: spirv::Word, coordinate: spirv::Word, dref: spirv::Word, image_operands: Option<spirv::ImageOperands>) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn image_sample_dref_explicit_lod(&mut self, result_type: spirv::Word, sampled_image: spirv::Word, coordinate: spirv::Word, dref: spirv::Word, image_operands: spirv::ImageOperands) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
        let inst = mr::Instruction::new(spirv::Op::ImageSampleDrefExplicitLod, Some(result_type), Some(id), vec![mr::Operand::IdRef(sampled_image), mr::Operand::IdRef(coordinate), mr::Operand::IdRef(dref), mr::Operand::ImageOperands(image_operands)]);
        self.basic_block.as_mut().unwrap().instructions.push(inst);
        Ok(id)
    }

    /// Appends an OpImageSampleProjImplicitLod instruction to the current basic block.
    pub fn image_sample_proj_implicit_lod(&mut self, result_type: spirv::Word, sampled_image: spirv::Word, coordinate: spirv::Word, image_operands: Option<spirv::ImageOperands>) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn image_sample_proj_explicit_lod(&mut self, result_type: spirv::Word, sampled_image: spirv::Word, coordinate: spirv::Word, image_operands: spirv::ImageOperands) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
        let inst = mr::Instruction::new(spirv::Op::ImageSampleProjExplicitLod, Some(result_type), Some(id), vec![mr::Operand::IdRef(sampled_image), mr::Operand::IdRef(coordinate), mr::Operand::ImageOperands(image_operands)]);
        self.basic_block.as_mut().unwrap().instructions.push(inst);
        Ok(id)
    }

    /// Appends an OpImageSampleProjDrefImplicitLod instruction to the current basic block.
    pub fn image_sample_proj_dref_implicit_lod(&mut self, result_type: spirv::Word, sampled_image: spirv::Word, coordinate: spirv::Word, dref: spirv::Word, image_operands: Option<spirv::ImageOperands>) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn image_sample_proj_dref_explicit_lod(&mut self, result_type: spirv::Word, sampled_image: spirv::Word, coordinate: spirv::Word, dref: spirv::Word, image_operands: spirv::ImageOperands) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
        let inst = mr::Instruction::new(spirv::Op::ImageSampleProjDrefExplicitLod, Some(result_type), Some(id), vec![mr::Operand::IdRef(sampled_image), mr::Operand::IdRef(coordinate), mr::Operand::IdRef(dref), mr::Operand::ImageOperands(image_operands)]);
        self.basic_block.as_mut().unwrap().instructions.push(inst);
        Ok(id)
    }

    /// Appends an OpImageFetch instruction to the current basic block.
    pub fn image_fetch(&mut self, result_type: spirv::Word, image: spirv::Word, coordinate: spirv::Word, image_operands: Option<spirv::ImageOperands>) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn image_gather(&mut self, result_type: spirv::Word, sampled_image: spirv::Word, coordinate: spirv::Word, component: spirv::Word, image_operands: Option<spirv::ImageOperands>) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn image_dref_gather(&mut self, result_type: spirv::Word, sampled_image: spirv::Word, coordinate: spirv::Word, dref: spirv::Word, image_operands: Option<spirv::ImageOperands>) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn image_read(&mut self, result_type: spirv::Word, image: spirv::Word, coordinate: spirv::Word, image_operands: Option<spirv::ImageOperands>) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn image_write(&mut self, image: spirv::Word, coordinate: spirv::Word, texel: spirv::Word, image_operands: Option<spirv::ImageOperands>) -> BuildResult<()> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
    ...
    pub fn image(&mut self, result_type: spirv::Word, sampled_image: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn image_query_format(&mut self, result_type: spirv::Word, image: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn image_query_order(&mut self, result_type: spirv::Word, image: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn image_query_size_lod(&mut self, result_type: spirv::Word, image: spirv::Word, level_of_detail: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn image_query_size(&mut self, result_type: spirv::Word, image: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn image_query_lod(&mut self, result_type: spirv::Word, sampled_image: spirv::Word, coordinate: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn image_query_levels(&mut self, result_type: spirv::Word, image: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn image_query_samples(&mut self, result_type: spirv::Word, image: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn convert_fto_u(&mut self, result_type: spirv::Word, float_value: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn convert_fto_s(&mut self, result_type: spirv::Word, float_value: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn convert_sto_f(&mut self, result_type: spirv::Word, signed_value: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn convert_uto_f(&mut self, result_type: spirv::Word, unsigned_value: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn uconvert(&mut self, result_type: spirv::Word, unsigned_value: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn sconvert(&mut self, result_type: spirv::Word, signed_value: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn fconvert(&mut self, result_type: spirv::Word, float_value: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn quantize_to_f16(&mut self, result_type: spirv::Word, value: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn convert_ptr_to_u(&mut self, result_type: spirv::Word, pointer: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn sat_convert_sto_u(&mut self, result_type: spirv::Word, signed_value: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn sat_convert_uto_s(&mut self, result_type: spirv::Word, unsigned_value: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn convert_uto_ptr(&mut self, result_type: spirv::Word, integer_value: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn ptr_cast_to_generic(&mut self, result_type: spirv::Word, pointer: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn generic_cast_to_ptr(&mut self, result_type: spirv::Word, pointer: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn generic_cast_to_ptr_explicit(&mut self, result_type: spirv::Word, pointer: spirv::Word, storage: spirv::StorageClass) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn bitcast(&mut self, result_type: spirv::Word, operand: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn snegate(&mut self, result_type: spirv::Word, operand: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn fnegate(&mut self, result_type: spirv::Word, operand: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn iadd(&mut self, result_type: spirv::Word, operand_1: spirv::Word, operand_2: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn fadd(&mut self, result_type: spirv::Word, operand_1: spirv::Word, operand_2: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn isub(&mut self, result_type: spirv::Word, operand_1: spirv::Word, operand_2: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn fsub(&mut self, result_type: spirv::Word, operand_1: spirv::Word, operand_2: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn imul(&mut self, result_type: spirv::Word, operand_1: spirv::Word, operand_2: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn fmul(&mut self, result_type: spirv::Word, operand_1: spirv::Word, operand_2: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn udiv(&mut self, result_type: spirv::Word, operand_1: spirv::Word, operand_2: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn sdiv(&mut self, result_type: spirv::Word, operand_1: spirv::Word, operand_2: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn fdiv(&mut self, result_type: spirv::Word, operand_1: spirv::Word, operand_2: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn umod(&mut self, result_type: spirv::Word, operand_1: spirv::Word, operand_2: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn srem(&mut self, result_type: spirv::Word, operand_1: spirv::Word, operand_2: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn smod(&mut self, result_type: spirv::Word, operand_1: spirv::Word, operand_2: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn frem(&mut self, result_type: spirv::Word, operand_1: spirv::Word, operand_2: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn fmod(&mut self, result_type: spirv::Word, operand_1: spirv::Word, operand_2: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn vector_times_scalar(&mut self, result_type: spirv::Word, vector: spirv::Word, scalar: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn matrix_times_scalar(&mut self, result_type: spirv::Word, matrix: spirv::Word, scalar: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn vector_times_matrix(&mut self, result_type: spirv::Word, vector: spirv::Word, matrix: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn matrix_times_vector(&mut self, result_type: spirv::Word, matrix: spirv::Word, vector: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn matrix_times_matrix(&mut self, result_type: spirv::Word, left_matrix: spirv::Word, right_matrix: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn outer_product(&mut self, result_type: spirv::Word, vector_1: spirv::Word, vector_2: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn dot(&mut self, result_type: spirv::Word, vector_1: spirv::Word, vector_2: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn iadd_carry(&mut self, result_type: spirv::Word, operand_1: spirv::Word, operand_2: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn isub_borrow(&mut self, result_type: spirv::Word, operand_1: spirv::Word, operand_2: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn umul_extended(&mut self, result_type: spirv::Word, operand_1: spirv::Word, operand_2: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn smul_extended(&mut self, result_type: spirv::Word, operand_1: spirv::Word, operand_2: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn any(&mut self, result_type: spirv::Word, vector: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn all(&mut self, result_type: spirv::Word, vector: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn is_nan(&mut self, result_type: spirv::Word, x: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn is_inf(&mut self, result_type: spirv::Word, x: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn is_finite(&mut self, result_type: spirv::Word, x: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn is_normal(&mut self, result_type: spirv::Word, x: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn sign_bit_set(&mut self, result_type: spirv::Word, x: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn less_or_greater(&mut self, result_type: spirv::Word, x: spirv::Word, y: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn ordered(&mut self, result_type: spirv::Word, x: spirv::Word, y: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn unordered(&mut self, result_type: spirv::Word, x: spirv::Word, y: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn logical_equal(&mut self, result_type: spirv::Word, operand_1: spirv::Word, operand_2: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn logical_not_equal(&mut self, result_type: spirv::Word, operand_1: spirv::Word, operand_2: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn logical_or(&mut self, result_type: spirv::Word, operand_1: spirv::Word, operand_2: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn logical_and(&mut self, result_type: spirv::Word, operand_1: spirv::Word, operand_2: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn logical_not(&mut self, result_type: spirv::Word, operand: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn select(&mut self, result_type: spirv::Word, condition: spirv::Word, object_1: spirv::Word, object_2: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn iequal(&mut self, result_type: spirv::Word, operand_1: spirv::Word, operand_2: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn inot_equal(&mut self, result_type: spirv::Word, operand_1: spirv::Word, operand_2: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn ugreater_than(&mut self, result_type: spirv::Word, operand_1: spirv::Word, operand_2: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn sgreater_than(&mut self, result_type: spirv::Word, operand_1: spirv::Word, operand_2: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn ugreater_than_equal(&mut self, result_type: spirv::Word, operand_1: spirv::Word, operand_2: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn sgreater_than_equal(&mut self, result_type: spirv::Word, operand_1: spirv::Word, operand_2: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn uless_than(&mut self, result_type: spirv::Word, operand_1: spirv::Word, operand_2: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn sless_than(&mut self, result_type: spirv::Word, operand_1: spirv::Word, operand_2: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn uless_than_equal(&mut self, result_type: spirv::Word, operand_1: spirv::Word, operand_2: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn sless_than_equal(&mut self, result_type: spirv::Word, operand_1: spirv::Word, operand_2: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn ford_equal(&mut self, result_type: spirv::Word, operand_1: spirv::Word, operand_2: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn funord_equal(&mut self, result_type: spirv::Word, operand_1: spirv::Word, operand_2: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn ford_not_equal(&mut self, result_type: spirv::Word, operand_1: spirv::Word, operand_2: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn funord_not_equal(&mut self, result_type: spirv::Word, operand_1: spirv::Word, operand_2: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn ford_less_than(&mut self, result_type: spirv::Word, operand_1: spirv::Word, operand_2: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn funord_less_than(&mut self, result_type: spirv::Word, operand_1: spirv::Word, operand_2: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn ford_greater_than(&mut self, result_type: spirv::Word, operand_1: spirv::Word, operand_2: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn funord_greater_than(&mut self, result_type: spirv::Word, operand_1: spirv::Word, operand_2: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn ford_less_than_equal(&mut self, result_type: spirv::Word, operand_1: spirv::Word, operand_2: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn funord_less_than_equal(&mut self, result_type: spirv::Word, operand_1: spirv::Word, operand_2: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn ford_greater_than_equal(&mut self, result_type: spirv::Word, operand_1: spirv::Word, operand_2: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn funord_greater_than_equal(&mut self, result_type: spirv::Word, operand_1: spirv::Word, operand_2: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn shift_right_logical(&mut self, result_type: spirv::Word, base: spirv::Word, shift: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn shift_right_arithmetic(&mut self, result_type: spirv::Word, base: spirv::Word, shift: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn shift_left_logical(&mut self, result_type: spirv::Word, base: spirv::Word, shift: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn bitwise_or(&mut self, result_type: spirv::Word, operand_1: spirv::Word, operand_2: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn bitwise_xor(&mut self, result_type: spirv::Word, operand_1: spirv::Word, operand_2: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn bitwise_and(&mut self, result_type: spirv::Word, operand_1: spirv::Word, operand_2: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn not(&mut self, result_type: spirv::Word, operand: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn bit_field_insert(&mut self, result_type: spirv::Word, base: spirv::Word, insert: spirv::Word, offset: spirv::Word, count: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn bit_field_sextract(&mut self, result_type: spirv::Word, base: spirv::Word, offset: spirv::Word, count: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn bit_field_uextract(&mut self, result_type: spirv::Word, base: spirv::Word, offset: spirv::Word, count: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn bit_reverse(&mut self, result_type: spirv::Word, base: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn bit_count(&mut self, result_type: spirv::Word, base: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn dpdx(&mut self, result_type: spirv::Word, p: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn dpdy(&mut self, result_type: spirv::Word, p: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn fwidth(&mut self, result_type: spirv::Word, p: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn dpdx_fine(&mut self, result_type: spirv::Word, p: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn dpdy_fine(&mut self, result_type: spirv::Word, p: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn fwidth_fine(&mut self, result_type: spirv::Word, p: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn dpdx_coarse(&mut self, result_type: spirv::Word, p: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn dpdy_coarse(&mut self, result_type: spirv::Word, p: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn fwidth_coarse(&mut self, result_type: spirv::Word, p: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn atomic_load(&mut self, result_type: spirv::Word, pointer: spirv::Word, scope: spirv::Word, semantics: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn atomic_exchange(&mut self, result_type: spirv::Word, pointer: spirv::Word, scope: spirv::Word, semantics: spirv::Word, value: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn atomic_compare_exchange(&mut self, result_type: spirv::Word, pointer: spirv::Word, scope: spirv::Word, equal: spirv::Word, unequal: spirv::Word, value: spirv::Word, comparator: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn atomic_compare_exchange_weak(&mut self, result_type: spirv::Word, pointer: spirv::Word, scope: spirv::Word, equal: spirv::Word, unequal: spirv::Word, value: spirv::Word, comparator: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn atomic_iincrement(&mut self, result_type: spirv::Word, pointer: spirv::Word, scope: spirv::Word, semantics: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn atomic_idecrement(&mut self, result_type: spirv::Word, pointer: spirv::Word, scope: spirv::Word, semantics: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn atomic_iadd(&mut self, result_type: spirv::Word, pointer: spirv::Word, scope: spirv::Word, semantics: spirv::Word, value: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn atomic_isub(&mut self, result_type: spirv::Word, pointer: spirv::Word, scope: spirv::Word, semantics: spirv::Word, value: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn atomic_smin(&mut self, result_type: spirv::Word, pointer: spirv::Word, scope: spirv::Word, semantics: spirv::Word, value: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn atomic_umin(&mut self, result_type: spirv::Word, pointer: spirv::Word, scope: spirv::Word, semantics: spirv::Word, value: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn atomic_smax(&mut self, result_type: spirv::Word, pointer: spirv::Word, scope: spirv::Word, semantics: spirv::Word, value: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn atomic_umax(&mut self, result_type: spirv::Word, pointer: spirv::Word, scope: spirv::Word, semantics: spirv::Word, value: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn atomic_and(&mut self, result_type: spirv::Word, pointer: spirv::Word, scope: spirv::Word, semantics: spirv::Word, value: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn atomic_or(&mut self, result_type: spirv::Word, pointer: spirv::Word, scope: spirv::Word, semantics: spirv::Word, value: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn atomic_xor(&mut self, result_type: spirv::Word, pointer: spirv::Word, scope: spirv::Word, semantics: spirv::Word, value: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn phi(&mut self, result_type: spirv::Word, variable_parent_: Vec<(spirv::Word, spirv::Word)>) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn loop_merge(&mut self, merge_block: spirv::Word, continue_target: spirv::Word, loop_control: spirv::LoopControl) -> BuildResult<()> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let inst = mr::Instruction::new(spirv::Op::LoopMerge, None, None, vec![mr::Operand::IdRef(merge_block), mr::Operand::IdRef(continue_target), mr::Operand::LoopControl(loop_control)]);
        Ok(self.basic_block.as_mut().unwrap().instructions.push(inst))
    }
    ...
    pub fn group_async_copy(&mut self, result_type: spirv::Word, execution: spirv::Word, destination: spirv::Word, source: spirv::Word, num_elements: spirv::Word, stride: spirv::Word, event: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn group_all(&mut self, result_type: spirv::Word, execution: spirv::Word, predicate: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn group_any(&mut self, result_type: spirv::Word, execution: spirv::Word, predicate: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn group_broadcast(&mut self, result_type: spirv::Word, execution: spirv::Word, value: spirv::Word, local_id: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn group_iadd(&mut self, result_type: spirv::Word, execution: spirv::Word, operation: spirv::GroupOperation, x: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn group_fadd(&mut self, result_type: spirv::Word, execution: spirv::Word, operation: spirv::GroupOperation, x: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn group_fmin(&mut self, result_type: spirv::Word, execution: spirv::Word, operation: spirv::GroupOperation, x: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn group_umin(&mut self, result_type: spirv::Word, execution: spirv::Word, operation: spirv::GroupOperation, x: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn group_smin(&mut self, result_type: spirv::Word, execution: spirv::Word, operation: spirv::GroupOperation, x: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn group_fmax(&mut self, result_type: spirv::Word, execution: spirv::Word, operation: spirv::GroupOperation, x: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn group_umax(&mut self, result_type: spirv::Word, execution: spirv::Word, operation: spirv::GroupOperation, x: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn group_smax(&mut self, result_type: spirv::Word, execution: spirv::Word, operation: spirv::GroupOperation, x: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn read_pipe(&mut self, result_type: spirv::Word, pipe: spirv::Word, pointer: spirv::Word, packet_size: spirv::Word, packet_alignment: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn write_pipe(&mut self, result_type: spirv::Word, pipe: spirv::Word, pointer: spirv::Word, packet_size: spirv::Word, packet_alignment: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn reserved_read_pipe(&mut self, result_type: spirv::Word, pipe: spirv::Word, reserve_id: spirv::Word, index: spirv::Word, pointer: spirv::Word, packet_size: spirv::Word, packet_alignment: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn reserved_write_pipe(&mut self, result_type: spirv::Word, pipe: spirv::Word, reserve_id: spirv::Word, index: spirv::Word, pointer: spirv::Word, packet_size: spirv::Word, packet_alignment: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn reserve_read_pipe_packets(&mut self, result_type: spirv::Word, pipe: spirv::Word, num_packets: spirv::Word, packet_size: spirv::Word, packet_alignment: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn reserve_write_pipe_packets(&mut self, result_type: spirv::Word, pipe: spirv::Word, num_packets: spirv::Word, packet_size: spirv::Word, packet_alignment: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn is_valid_reserve_id(&mut self, result_type: spirv::Word, reserve_id: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn get_num_pipe_packets(&mut self, result_type: spirv::Word, pipe: spirv::Word, packet_size: spirv::Word, packet_alignment: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn get_max_pipe_packets(&mut self, result_type: spirv::Word, pipe: spirv::Word, packet_size: spirv::Word, packet_alignment: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn group_reserve_read_pipe_packets(&mut self, result_type: spirv::Word, execution: spirv::Word, pipe: spirv::Word, num_packets: spirv::Word, packet_size: spirv::Word, packet_alignment: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn group_reserve_write_pipe_packets(&mut self, result_type: spirv::Word, execution: spirv::Word, pipe: spirv::Word, num_packets: spirv::Word, packet_size: spirv::Word, packet_alignment: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn enqueue_marker(&mut self, result_type: spirv::Word, queue: spirv::Word, num_events: spirv::Word, wait_events: spirv::Word, ret_event: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn enqueue_kernel(&mut self, result_type: spirv::Word, queue: spirv::Word, flags: spirv::Word, nd_range: spirv::Word, num_events: spirv::Word, wait_events: spirv::Word, ret_event: spirv::Word, invoke: spirv::Word, param: spirv::Word, param_size: spirv::Word, param_align: spirv::Word, local_size: Vec<spirv::Word>) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn get_kernel_ndrange_sub_group_count(&mut self, result_type: spirv::Word, nd_range: spirv::Word, invoke: spirv::Word, param: spirv::Word, param_size: spirv::Word, param_align: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn get_kernel_ndrange_max_sub_group_size(&mut self, result_type: spirv::Word, nd_range: spirv::Word, invoke: spirv::Word, param: spirv::Word, param_size: spirv::Word, param_align: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn get_kernel_work_group_size(&mut self, result_type: spirv::Word, invoke: spirv::Word, param: spirv::Word, param_size: spirv::Word, param_align: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn get_kernel_preferred_work_group_size_multiple(&mut self, result_type: spirv::Word, invoke: spirv::Word, param: spirv::Word, param_size: spirv::Word, param_align: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn create_user_event(&mut self, result_type: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn is_valid_event(&mut self, result_type: spirv::Word, event: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn get_default_queue(&mut self, result_type: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn build_ndrange(&mut self, result_type: spirv::Word, global_work_size: spirv::Word, local_work_size: spirv::Word, global_work_offset: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn image_sparse_sample_implicit_lod(&mut self, result_type: spirv::Word, sampled_image: spirv::Word, coordinate: spirv::Word, image_operands: Option<spirv::ImageOperands>) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn image_sparse_sample_explicit_lod(&mut self, result_type: spirv::Word, sampled_image: spirv::Word, coordinate: spirv::Word, image_operands: spirv::ImageOperands) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
        let inst = mr::Instruction::new(spirv::Op::ImageSparseSampleExplicitLod, Some(result_type), Some(id), vec![mr::Operand::IdRef(sampled_image), mr::Operand::IdRef(coordinate), mr::Operand::ImageOperands(image_operands)]);
        self.basic_block.as_mut().unwrap().instructions.push(inst);
        Ok(id)
    }

    /// Appends an OpImageSparseSampleDrefImplicitLod instruction to the current basic block.
    pub fn image_sparse_sample_dref_implicit_lod(&mut self, result_type: spirv::Word, sampled_image: spirv::Word, coordinate: spirv::Word, dref: spirv::Word, image_operands: Option<spirv::ImageOperands>) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn image_sparse_sample_dref_explicit_lod(&mut self, result_type: spirv::Word, sampled_image: spirv::Word, coordinate: spirv::Word, dref: spirv::Word, image_operands: spirv::ImageOperands) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
        let inst = mr::Instruction::new(spirv::Op::ImageSparseSampleDrefExplicitLod, Some(result_type), Some(id), vec![mr::Operand::IdRef(sampled_image), mr::Operand::IdRef(coordinate), mr::Operand::IdRef(dref), mr::Operand::ImageOperands(image_operands)]);
        self.basic_block.as_mut().unwrap().instructions.push(inst);
        Ok(id)
    }

    /// Appends an OpImageSparseSampleProjImplicitLod instruction to the current basic block.
    pub fn image_sparse_sample_proj_implicit_lod(&mut self, result_type: spirv::Word, sampled_image: spirv::Word, coordinate: spirv::Word, image_operands: Option<spirv::ImageOperands>) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn image_sparse_sample_proj_explicit_lod(&mut self, result_type: spirv::Word, sampled_image: spirv::Word, coordinate: spirv::Word, image_operands: spirv::ImageOperands) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
        let inst = mr::Instruction::new(spirv::Op::ImageSparseSampleProjExplicitLod, Some(result_type), Some(id), vec![mr::Operand::IdRef(sampled_image), mr::Operand::IdRef(coordinate), mr::Operand::ImageOperands(image_operands)]);
        self.basic_block.as_mut().unwrap().instructions.push(inst);
        Ok(id)
    }

    /// Appends an OpImageSparseSampleProjDrefImplicitLod instruction to the current basic block.
    pub fn image_sparse_sample_proj_dref_implicit_lod(&mut self, result_type: spirv::Word, sampled_image: spirv::Word, coordinate: spirv::Word, dref: spirv::Word, image_operands: Option<spirv::ImageOperands>) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn image_sparse_sample_proj_dref_explicit_lod(&mut self, result_type: spirv::Word, sampled_image: spirv::Word, coordinate: spirv::Word, dref: spirv::Word, image_operands: spirv::ImageOperands) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
        let inst = mr::Instruction::new(spirv::Op::ImageSparseSampleProjDrefExplicitLod, Some(result_type), Some(id), vec![mr::Operand::IdRef(sampled_image), mr::Operand::IdRef(coordinate), mr::Operand::IdRef(dref), mr::Operand::ImageOperands(image_operands)]);
        self.basic_block.as_mut().unwrap().instructions.push(inst);
        Ok(id)
    }

    /// Appends an OpImageSparseFetch instruction to the current basic block.
    pub fn image_sparse_fetch(&mut self, result_type: spirv::Word, image: spirv::Word, coordinate: spirv::Word, image_operands: Option<spirv::ImageOperands>) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn image_sparse_gather(&mut self, result_type: spirv::Word, sampled_image: spirv::Word, coordinate: spirv::Word, component: spirv::Word, image_operands: Option<spirv::ImageOperands>) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn image_sparse_dref_gather(&mut self, result_type: spirv::Word, sampled_image: spirv::Word, coordinate: spirv::Word, dref: spirv::Word, image_operands: Option<spirv::ImageOperands>) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn image_sparse_texels_resident(&mut self, result_type: spirv::Word, resident_code: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn atomic_flag_test_and_set(&mut self, result_type: spirv::Word, pointer: spirv::Word, scope: spirv::Word, semantics: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn image_sparse_read(&mut self, result_type: spirv::Word, image: spirv::Word, coordinate: spirv::Word, image_operands: Option<spirv::ImageOperands>) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn size_of(&mut self, result_type: spirv::Word, pointer: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn create_pipe_from_pipe_storage(&mut self, result_type: spirv::Word, pipe_storage: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn get_kernel_local_size_for_subgroup_count(&mut self, result_type: spirv::Word, subgroup_count: spirv::Word, invoke: spirv::Word, param: spirv::Word, param_size: spirv::Word, param_align: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn get_kernel_max_num_subgroups(&mut self, result_type: spirv::Word, invoke: spirv::Word, param: spirv::Word, param_size: spirv::Word, param_align: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn named_barrier_initialize(&mut self, result_type: spirv::Word, subgroup_count: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn subgroup_ballot_khr(&mut self, result_type: spirv::Word, predicate: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
    ...
    pub fn subgroup_first_invocation_khr(&mut self, result_type: spirv::Word, value: spirv::Word) -> BuildResult<spirv::Word> {
        if self.basic_block.is_none() {
            return Err(Error::DetachedInstruction);
        }
        let id = self.id();
