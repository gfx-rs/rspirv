...
impl Builder {
    ...
    pub fn constant_true(&mut self, result_type: spirv::Word) -> spirv::Word {
        let id = self.id();
        let inst = dr::Instruction::new(spirv::Op::ConstantTrue, Some(result_type), Some(id), vec![]);
    ...
    pub fn constant_false(&mut self, result_type: spirv::Word) -> spirv::Word {
        let id = self.id();
        let inst = dr::Instruction::new(spirv::Op::ConstantFalse, Some(result_type), Some(id), vec![]);
        self.module.types_global_values.push(inst);
        id
    }
    ...
    pub fn constant_sampler(&mut self, result_type: spirv::Word, sampler_addressing_mode: spirv::SamplerAddressingMode, param: u32, sampler_filter_mode: spirv::SamplerFilterMode) -> spirv::Word {
        let id = self.id();
        let inst = dr::Instruction::new(spirv::Op::ConstantSampler, Some(result_type), Some(id), vec![dr::Operand::SamplerAddressingMode(sampler_addressing_mode), dr::Operand::LiteralInt32(param), dr::Operand::SamplerFilterMode(sampler_filter_mode)]);
        self.module.types_global_values.push(inst);
        id
    }
    ...
    pub fn constant_null(&mut self, result_type: spirv::Word) -> spirv::Word {
        let id = self.id();
        let inst = dr::Instruction::new(spirv::Op::ConstantNull, Some(result_type), Some(id), vec![]);
        self.module.types_global_values.push(inst);
        id
    }
    ...
    pub fn spec_constant_true(&mut self, result_type: spirv::Word) -> spirv::Word {
        let id = self.id();
        let inst = dr::Instruction::new(spirv::Op::SpecConstantTrue, Some(result_type), Some(id), vec![]);
        self.module.types_global_values.push(inst);
        id
    }
    ...
    pub fn spec_constant_false(&mut self, result_type: spirv::Word) -> spirv::Word {
        let id = self.id();
        let inst = dr::Instruction::new(spirv::Op::SpecConstantFalse, Some(result_type), Some(id), vec![]);
        self.module.types_global_values.push(inst);
        id
    }
    ...
    pub fn spec_constant_op(&mut self, result_type: spirv::Word, opcode: spirv::Op) -> spirv::Word {
        let id = self.id();
        let inst = dr::Instruction::new(spirv::Op::SpecConstantOp, Some(result_type), Some(id), vec![dr::Operand::LiteralSpecConstantOpInteger(opcode)]);
        self.module.types_global_values.push(inst);
        id
    }
    ...
    pub fn constant_pipe_storage(&mut self, result_type: spirv::Word, packet_size: u32, packet_alignment: u32, capacity: u32) -> spirv::Word {
        let id = self.id();
        let inst = dr::Instruction::new(spirv::Op::ConstantPipeStorage, Some(result_type), Some(id), vec![dr::Operand::LiteralInt32(packet_size), dr::Operand::LiteralInt32(packet_alignment), dr::Operand::LiteralInt32(capacity)]);
        self.module.types_global_values.push(inst);
        id
    }
}